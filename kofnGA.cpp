...
arma::urowvec order_fun(arma::rowvec arma_vec) {
  NumericVector rcpp_vec = wrap(arma_vec) ;
 ...
 *  This function is used to imitate the R code: t(replicate(popsize,sample(indices,k)))
 *  which is used in the construction of pop if initpop is null.
 *  Inputs:
 *  rows      The number of rows in pop.
 *  cols      The number of columns in pop.
 *  indices   A vector of integers to sample from.
 */
// [[Rcpp::export]]
IntegerMatrix rep_transpose_start(int rows, int cols, IntegerVector indices) {
  IntegerMatrix pop = no_init_matrix( rows , cols );
  for (arma::uword i = 0; i < rows; i++) {
    pop( i , _ ) = Rcpp::sample(indices, cols);
  }
  return pop;
}
 ...
 *  indices   A vector of integers to sample from.
 */
arma::umat rep_transpose(arma::umat mat, int rows, int cols, arma::urowvec indices) {
  for (arma::uword i = 0; i < rows; i++) {
    mat.row(i) = Rcpp::RcppArmadillo::sample(indices, cols, false);
  }
  return mat;
}
...
Rcpp::List kofnGAv1(int n, int k, int popsize,int keepbest,int ngen,
  ...
  // ObjFun_v1_vec
  IntegerVector indicesRcpp = Rcpp::seq(0,n-1);
  arma::urowvec indicesArma = as<arma::urowvec>(indicesRcpp); 
  ...
  if (keepbest > 0) {
    IntegerVector elitespotsRcpp = Rcpp::seq(0,keepbest-1);
    elitespotsArma = as<arma::urowvec>(elitespotsRcpp); 
    
    IntegerVector newspotsRcpp = Rcpp::seq(keepbest,popsize-1);
    newspotsArma = as<arma::urowvec>(newspotsRcpp);  
  }
  
  arma::rowvec fitness_old = arma::rowvec(popsize);
  arma::rowvec fitness_new = arma::rowvec(popsize);
  arma::umat offspring = arma::umat(popsize, k); 
  arma::umat old_best = arma::umat(ngen+1, k); 
  arma::vec old_obj = arma::vec(ngen+1);
  arma::vec old_avg = arma::vec(ngen+1);
  arma::umat Tourneys1 = arma::umat(popsize, tourneysize); 
  arma::umat Tourneys2 = arma::umat(popsize, tourneysize);  
  ...
  // Note:
  // old_best.row(0) = arma::sort(Arma_pop.rows(pop_row_ids));
  ...
  old_best.row(0) = Arma_pop.rows(pop_row_ids);
  old_best.row(0) = arma::sort(old_best.row(0));
  
  old_obj(0) = arma::min(fitness_old);
  
  old_avg(0) = arma::mean(fitness_old);
  
  //=== Loop through generations ============================================
  IntegerVector tourney_indices = Rcpp::seq(0,popsize-1);
  arma::urowvec Arma_tourney_indices = as<arma::urowvec>(tourney_indices); // ADDED
  ...
  for (uword gen = 0; gen < ngen; gen++) {
    ...
    Tourneys1 = rep_transpose(tourneys1_to_transpose, popsize, tourneysize, Arma_tourney_indices);
    Tourneys2 = rep_transpose(tourneys2_to_transpose, popsize, tourneysize, Arma_tourney_indices);
    
    for (arma::uword i = 0; i < popsize; i++) {
      // Parents1
      arma::rowvec fitness_old_sliced = fitness_old.cols(Tourneys1.row(i));
      ...
      // Parents2
      arma::rowvec fitness_old_sliced_T2 = fitness_old.cols(Tourneys2.row(i));
    ...
    for (arma::uword i = 0; i < popsize; i++) {
      ...
      if (nchosen(i) > 0) {
        ...
        IntegerVector indices_clone = clone(indicesRcpp);
    ...
    rank_fitness_old = as<arma::urowvec>(rank_rcpp_fitness_old);
    pop_row_ids = arma::find(rank_fitness_old == 1);
    // Note:
    // old_best.row(gen+1) = arma::sort(Arma_pop.rows(pop_row_ids));
    // If I only use the above line, a bug occurs and the output is NOT
    // guaranteed to be sorted each time. Hence the two lines below instead:
    old_best.row(gen+1) = Arma_pop.rows(pop_row_ids);
    old_best.row(gen+1) = arma::sort(old_best.row(gen+1));
    
    old_obj(gen+1) = arma::min(fitness_old);
    
    old_avg(gen+1) = arma::mean(fitness_old);
  }
  ...
  arma::uvec alltimebest = arma::find(old_obj == arma::min(old_obj));
  alltimebest = alltimebest.tail(1);
  
  old_best += 1;
  arma::umat sub_mat_old_best = old_best.rows(alltimebest);
  
  arma::rowvec sub_vec_old_obj = old_obj.elem(alltimebest);
